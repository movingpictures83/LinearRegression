...
def raw_dataframe(inputfile):
  """Load the automobile data set as a pd.DataFrame."""
  # Download and cache the data
  #path = tf.keras.utils.get_file(URL.split("/")[-1], URL)

  # Load it into a pandas DataFrame
  df = pd.read_csv(inputfile, names=COLUMN_TYPES.keys(),
                   dtype=COLUMN_TYPES, na_values="?")

  return df


def load_data(inputfile,y_name="price", train_fraction=0.7, seed=None):
